...
const flawlessMap = {
  ...
  'work_chiba_metro_cor_120': 'https://images.unsplash.com/photo-1578637387939-43c525550085?q=80&w=1600&auto=format&fit=crop',
...
async function run() {
  console.log(`Downloading all ${projects.length} project images...`);
  
  const tasks = projects.map(async (p, i) => {
    const dest = `./public/images/${p.id}.jpg`;
    const url = flawlessMap[p.id];
    try {
